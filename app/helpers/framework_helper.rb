...
module FrameworkHelper
  ...
  PAGE_DESCRIPTIONS = {
    ...
    'rendering_modes' => 'The grayscale tiers and color modes a screen can carry, what each one paints, and the bit depth JavaScript can read',
    ...
    'themes' => 'Visually customize any plugin with a drop-in stylesheet that recolors the whole screen',
    ...
    'position' => 'Take an element out of flow and set its distance from each edge of its container',
    ...
    'image' => 'Place images with size, object fit, dithering, inversion, and adaptive icon utilities',
    ...
    'outline' => 'Draw a pixel-perfect dotted rounded border on any element',
    ...
    'framework_runtime' => 'The JavaScript pass that measures the screen and fits your content into it at render time',
  ...
  def framework_intro_paragraphs
    @framework_intro_paragraphs ||= {
      ...
      'devices' => 'A device profile makes a screen render true to one panel: its exact dimensions, pixel density, and interface scale. Add one class, screen--{keyname}, and a screen renders with all of it. The profiles come from TRMNL\'s own device list, so each class matches a real panel.',
      'rendering_modes' => 'A rendering mode is the class that tells a screen what its panel can print. Grayscale panels pick one of three bit-depth classes; color panels pick the class for their palette. Everything else follows that one class, from the dither patterns to the colors JavaScript reads.',
      ...
      'position' => 'Use Position to put one element over another instead of beside it. Offsets on the spacing scale hold it a set distance from the edges of its container, and a short stacking scale says which of two overlapping elements is drawn on top.',
      ...
      'outline' => 'The Outline utility draws a pixel-perfect dotted rounded border on any element. On 1-bit displays it places single-pixel dots at exact integer coordinates with pure CSS gradients; on 2-bit, 4-bit, and full-color displays it draws a standard CSS border with border-radius instead.',
      ...
      'divider' => 'The Divider element provides a simple, standalone shorthand for horizontal and vertical separators. It draws the same line as the border utilities at level 6, the line the steps 60 and 65 draw.',
      ...
      'inverse' => 'The Inverse utility flips one element and its children to the opposite color scheme: light content on a dark surface, or the reverse. Use it to make one element stand out, an active item or a selected row, without touching its siblings.',
      ...
      'v3_overview' => 'Framework 3.3 adds adaptive maps, position utilities for content that sits over other content, and a theme contract that reaches the title bar, item cards, structure and text. It builds on the 3.2 open-source release: themes, a JavaScript paint API, adaptive charts and icons, rebuilt borders and strokes, Text Scale, complete Scale modifiers, and Fluid Mashups. Existing markup keeps working; every 3.2 and 3.3 feature is opt-in.',
